...
class StudentExamResultsNaplanNumeracy(Base):
    ...
    student = relationship("Student")
class StudentExamResultsNaplanLanguageConventions(Base):
    __tablename__ = "student_exam_results_naplan_language_conventions"
    ...
    exam_attempt_id = Column(
        Integer,
        ForeignKey(
            "student_exam_naplan_language_conventions.id",
            ondelete="CASCADE"
        ),
        nullable=False
    )
...
def delete_exam_attempt(payload: dict, db: Session = Depends(get_db)):
    student_external_id = payload.get("student_id")  # e.g. Gem002
    ...
    try:
        ...
        if exam_type == "thinking_skills":
            ...
            if not latest_attempt:
                raise HTTPException(
                    status_code=404,
                    detail="No attempt found for today"
                )

            # STEP A: Delete responses
        ...
        for b in blocks:
            ...
            if isinstance(b, str):
                ...
                if block_type == "text":
                    ...
                    # structured blocks pass through unchanged
                    normalized.append(b)
...
def get_exam_review_oc_thinking_skills(
    ...
    responses = (
        ...
        .filter(
            StudentExamResponseOCThinkingSkills.exam_attempt_id == exam_attempt_id
        )
        .order_by(StudentExamResponseOCThinkingSkills.q_id)
        .all()
    )

    print(f"📊 OC response rows fetched: {len(responses)}")

    if not responses:
        print("⚠️ No OC responses found for exam_attempt_id =", exam_attempt_id)
        return {
            "exam_attempt_id": exam_attempt_id,
            "questions": []
        }
    ...
    if not exam:
        print("❌ OC Exam NOT FOUND for exam_id =", attempt.exam_id)
        raise HTTPException(
            status_code=404,
            detail="OC exam not found"
        )
...
@app.post("/api/quizzes-naplan-reading")
def create_naplan_reading_quiz(
    quiz: NaplanQuizCreate,
    db: Session = Depends(get_db),
):
    print("\n========== NAPLAN READING QUIZ CREATION START ==========")
    ...
    try:
        ...
        print("\n🧹 Deleting existing NAPLAN Reading quiz configs...")
        ...
        print("\n--- Creating quiz_naplan_reading row ---")

        new_quiz = QuizNaplanReading(
            class_name=quiz.class_name,        # e.g. "year 3"
            subject="reading",
            year=quiz.year,
            difficulty=quiz.difficulty,
        ...
        print("✅ NAPLAN READING QUIZ CREATED:", new_quiz)
        print("========== NAPLAN READING QUIZ CREATION COMPLETE ==========\n")

        return {
            "message": "NAPLAN Reading quiz created successfully",
            "quiz_id": new_quiz.id,
        }
...
@app.get("/api/exams/oc-reading-report")
def get_oc_reading_report(
    session_id: int = Query(..., description="OC Reading Exam session ID"),
    db: Session = Depends(get_db)
):
    ...
    # 1️⃣ Load session (OC)
    # --------------------------------------------------
    print("🟡 STEP 1: Loading StudentExamReadingOC session")
    ...
    if session.report_json:
        ...
        print("================ END GET OC READING REPORT ================\n")
        return session.report_json

    print("⚠️ report_json MISSING — entering diagnostic mode")

    # --------------------------------------------------
    # 4️⃣ Inspect per-question report rows (OC table)
    # --------------------------------------------------
    print("🟡 STEP 4: Counting StudentExamReportOCReading rows")
    ...
    print("📝 StudentExamReportOCReading row count:", answer_count)
...
def get_naplan_reading_report(
    ...
    if not attempt:
        print("❌ No completed Reading attempt found for student_id:", student.id)
        raise HTTPException(
            status_code=404,
            detail="No completed NAPLAN Reading exam found"
        )
    ...
    if not responses:
        ...
        raise HTTPException(
            status_code=404,
            detail="No responses found for NAPLAN Reading exam"
        )
...
def get_naplan_numeracy_report(
    ...
    print(f"👤 Resolved student: db_id={student.id}, year={student.student_year}")
...
def get_oc_mathematical_reasoning_report(
    ...
    if not responses:
        raise HTTPException(
            status_code=404,
            detail="No responses found for OC Mathematical Reasoning exam"
        )
...
def submit_oc_reading_exam(payload: dict, db: Session = Depends(get_db)):
    ...
    try:
        ...
        # 2️⃣ Load exam
        # --------------------------------------------------
        print("🟡 STEP 2: Loading GeneratedExamReading")
        ...
        for section in sections:
            ...
            for q in questions:
                ...
                db.add(StudentExamReportOCReading(
                    student_id=session.student_id,
                    exam_id=session.exam_id,
                    session_id=session.id,
                    topic=topic,
...
def archive_writing_response_to_admin_table(
    db: Session,
    student: Student,
    ...
    admin_record = AdminExamResponseWriting(
        ...
        topic=writing_response.topic,
        essay_text=writing_response.essay_text,

        writing_score=writing_response.writing_score,
        readiness_band=writing_response.readiness_band,

        ai_evaluation_json=exam_state.ai_evaluation_json
    )
...
def normalize_naplan_language_conventions_questions_live(
    ...
    for q in raw_questions or []:
        ...
        # --------------------------------------------------
        print("\n[FINAL PAYLOAD]")
        print("FINAL question_text:", repr(payload["question_text"]))
        print("FINAL question_blocks:")
        for b in payload["question_blocks"]:
            if b.get("type") == "text":
                print("TEXT:", repr(b.get("content")))
            else:
...
def finish_naplan_language_conventions_exam(
    ...
    print("🔔 FINISH NAPLAN LANGUAGE CONVENTIONS EXAM CALLED")
    print("📦 RAW PAYLOAD:", payload)

    # --------------------------------------------------
    # 0. Extract + validate student_id
    # --------------------------------------------------
    ...
    # --------------------------------------------------
    # 2. Fetch latest unfinished attempt
    # --------------------------------------------------
    attempt = (
        db.query(StudentExamNaplanLanguageConventions)
        .filter(
    ...
    if not exam:
        print("❌ Exam not found for attempt:", attempt.id)
        raise HTTPException(
            status_code=404,
            detail="Exam not found for this attempt"
        )
...
def finish_oc_thinking_skills_exam(
    ...
    print("\n================ FINISH OC THINKING SKILLS EXAM START ================")
    print("📥 Incoming payload:", req.dict())
    ...
    # --------------------------------------------------
    # 2️⃣ Fetch MOST RECENT OC attempt
    # --------------------------------------------------
    attempt = (
        db.query(StudentExamOCThinkingSkills)
        .filter(StudentExamOCThinkingSkills.student_id == student.id)
    ...
    if not attempt:
        print("❌ ABORT: No OC attempt exists for student")
        raise HTTPException(
            status_code=404,
            detail="No OC Thinking Skills exam attempt found"
        )

    print("✅ Latest OC attempt found:", {
        "attempt_id": attempt.id,
        "exam_id": attempt.exam_id,
        "started_at": attempt.started_at,
        "completed_at": attempt.completed_at
    ...
    if not exam:
        print("❌ ABORT: OC Exam not found or subject mismatch")
        raise HTTPException(status_code=404, detail="Exam not found")
    ...
    for q_id_str, selected in req.answers.items():
        ...
        response = (
            ...
            .filter(
                StudentExamResponseOCThinkingSkills.exam_attempt_id == attempt.id,
                StudentExamResponseOCThinkingSkills.q_id == q_id
            )
            .first()
        )

        if not response:
            print("❌ OC Response row missing:", {
                "attempt_id": attempt.id,
                "q_id": q_id
            })
            continue
    ...
    for block in blocks:
        if block["type"] == "text":
            ...
            if is_exam_start(block):
                inside_exam = True
                cleaned = strip_exam_markers(text).strip()
                if cleaned:
                    current = [{
                        **block,
                        "content": cleaned
...
def extract_reference_images(
    ...
    *,
    ...
    print(
        f"[{request_id}] 🧪 extract_reference_images RESULT | "
        f"count={len(reference_images)} | "
        f"images={reference_images}"
    )
...
def debug_detect_question_types(question_block):
    ...
    for item in question_block:
        ...
        text = (item.get("content") or "").strip().lower()

        match = re.search(r"question_type\s*:\s*(\d+)", text)
        if match:
